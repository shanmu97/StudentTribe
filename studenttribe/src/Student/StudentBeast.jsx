...
const CurvedCarousel = ({ products }) => {
  ...
  const [isCarouselHovered, setIsCarouselHovered] = useState(false); // New state for carousel hover
  ...
  const handleMouseUp = () => {
    setIsDragging(false);
    // Restart auto rotation after a delay only if carousel is not hovered
  ...
  const handleMouseLeave = () => {
    setIsDragging(false);
    setIsCarouselHovered(false); // Reset carousel hover state
  ...
  return (
    <div
      ...
      onMouseLeave={handleCarouselMouseLeave} // Updated to use carousel-specific handler
      onMouseEnter={handleCarouselMouseEnter} // New hover handler
    ...
    >
      ...
      <div className="absolute bottom-8 left-1/2 transform -translate-x-1/2 text-white/60 text-sm">
        Auto-rotating carousel • Drag to pause • Hover to pause
      </div>
    </div>
  );
};
...
const StudentBeast = () => {
  ...
  // MainScreen radial gradient background - Updated for full screen
  const gradientBg = (
    <div className="mainscreen-gradient-bg" style={{
      position: 'fixed', // Changed from absolute to fixed
      inset: 0,
      zIndex: -1, // Changed to negative z-index to stay behind content
      width: '100vw',
      height: '100vh',
      pointerEvents: 'none',
  ...
  return (
    ...
    >
      ...
      >
        {/* Title Section */}
        <div ref={titleRef} className="text-center text-white -mb-40">
        ...
        {/* 3D Curved Carousel - Reduced bottom spacing */}
        <div
          ref={carouselRef}
          className="relative flex items-center justify-center min-h-[900px] px-8" // Changed from pb-32 to pb-8
        >
